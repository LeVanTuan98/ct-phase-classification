...
from models.feature_extraction.densenet import densenet121, densenet161, densenet169, densenet201
...
class ImageClassifier(nn.Module):
    def __init__(self, cfg):
        ...
        # print(cfg)
#         if self.n_classes == 2 and 'softmax' in cfg._fields and cfg.softmax == False:     # if binary classification
#             self.n_classes = 1      # number of network's output is 1.
        ...
        # Set the classifier
        if cfg.conv_fc:
            fc = getattr(models.common, 'conv1x1')
        else:
...
class PLImageClassifier(pl.LightningModule):
    def __init__(self, cfg, device='cpu'):
        ...
        if cfg['loss'] == 'BCELogLossV2':
            self.loss_fn = getattr(losses, cfg['loss'])(device=device)
        else:
            self.loss_fn = getattr(torch.nn, cfg['loss'])()
        #print(cfg)
        if len(cfg['model_params']['labels']) == 2 and 'softmax' in cfg['model_params'].keys() and cfg['model_params']['softmax'] == False: 
