...
def gen_adj_num(labels, agree_rate=0.5, csv_path='/u01/data/Vimmec_Data_small_v4/train.csv', t=0.4):
...
if __name__ == "__main__":
    adj = gen_adj_num(labels='0,1,2,3,4,5', csv_path='D:/Dataset/Vinmec/v7.3/train_sss_3.csv')
    print(adj)  # import torch.nn as nn

    # x = torch.randn([2, 3, 2, 2])  # y = torch.randn([3, 2, 2])  # z = torch.matmul(x, y)  # print(z.size())

    # gcn1 = GraphConvolution(10, 100, 6)  # gcn2 = GraphConvolution(100, 6)  # x = torch.randn([2, 1, 10, 10])  # # x1,x2,x3,x4 = x.size()  # # print(x1, x2, x3, x4)  # # x = x.view((2, 6, -1))  # # print(x.size())  # # y = torch.randn([6, 14])  # #  # # z = torch.matmul(y, x)  # # print(z.size())  #  # adj = torch.randn([6, 6])  #  # embedded1 = gcn1(x, adj)  # print(embedded1.size())  # embedded1 = gcn2(embedded1, adj)  # print(embedded1.size())  #  # avgpool = nn.AdaptiveAvgPool2d((1, 1))  # embedded1 = avgpool(embedded1)  # print(embedded1.size())

    # df_valid = pd.read_csv('D:/Dataset/Vinmec/Noise/train_4.4.csv')  # df_valid = df_valid.fillna(0)  # label_names_0 = ['Pleural_Effusion', 'Cardiomegaly', 'Pneumothorax', 'Lung_Lesion',  #                  'Airspace_Opacity', 'Fracture']  # # if len(list(map(int, cfg.extract_fields.split(',')))) == 6:  # #     label_names_0 = ['Pleural_Effusion', 'Cardiomegaly', 'Pneumothorax', 'Lung_Lesion',  # #                      'Airspace_Opacity', 'Fracture']  # # else:  # print(list(df_valid)[0])  # df_valid = df_valid[label_names_0]  # print(df_valid)  #  # # indices = [0, 1,2,3,4,5,6]  # # label_names_0 = [list(df_valid)[idx] for idx in indices]  # #  # # df_valid = df_valid[label_names_0]  # # print(df_valid[list(df_valid)[1:]])  #  # data_matric = df_valid.iloc[:].copy().values  # (dataset_len,14)  # data_matric = np.array(data_matric, dtype=np.long)  # data_matric = np.abs(data_matric)  # _number = []  # datasize, number_labels = data_matric.shape  # for i in range(number_labels):  #     nozero = np.count_nonzero(data_matric[:, i])  #     _number.append(nozero)  # _nums = np.array(_number)  # _nums = _nums[:, np.newaxis]  # correlation_matrix = np.zeros((number_labels, number_labels), dtype=int)  # for i in range(number_labels):  #     for j in range(i + 1, number_labels):  #         a = data_matric[:, i]  #         b = data_matric[:, j]  #         c = a & b  #         num = np.count_nonzero(c)  #         correlation_matrix[i][j] = num  #         correlation_matrix[j][i] = num  # _adj = correlation_matrix  # _adj = _adj / _nums  # _adj[_adj < 0.05] = 0  # _adj[_adj >= 0.05] = 1  # _adj = _adj * 0.25 / (_adj.sum(0) + 1e-6)  # _adj = _adj + np.identity(number_labels, np.int)  #  # print(_adj)

    # a = torch.tensor([[2,3], [1,2]])  # print(a)  # b = torch.transpose(a, 0, 1)  # print(b)
