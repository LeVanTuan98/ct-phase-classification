...
class AttentionMap(nn.Module):
    ...
    def __init__(self, cfg, num_channels):
        ...
        self.cfg = cfg
        self.channel_attention = SEModule(num_channels)
        self.spatial_attention = SAModule(num_channels)
        self.pyramid_attention = FPAModule(num_channels, cfg.norm_type)
        self.cam_attention = CAM_Module(num_channels)
        self.strip_attention = StripPooling(num_channels)
    ...
    def forward(self, feat_map):
        ...
        elif self.cfg.attention_map == "DAA":
            return self.cam_attention(feat_map) + self.spatial_attention(feat_map)
        elif self.cfg.attention_map == "DACAM":
            return self.cam_attention(feat_map)
        elif self.cfg.attention_map == "STRIP":
            return self.strip_attention(feat_map)
